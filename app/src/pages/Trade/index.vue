  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        ...
        <ul
          class="list clearFix"
          v-for="(detailList, index) in tradeInfo.detailArrayList"
          :key="index"
        >
          <li>
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("userTradeInfo");
  },
  computed: {
    ...mapState({
  ...
  methods: {
    ...
    async submitOrder() {
      try {
        ...
        let result = await this.$API.reqSubmitOrder(tradeNo, userOrderInfo);
        if (result.code == 200) {
          this.orderId = result.data;
          this.$router.push(`/pay?orderId=${this.orderId}`);
        } else {
          alert(new Error(result.message));
        }
      } catch (error) {
        alert(error.message);
      }
